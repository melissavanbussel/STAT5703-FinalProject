...
A few of the risk adjustments are negative: 

```{r}
results <- audit[which(audit$RISK_Adjustment < 0), -c(1, 2, 5, 8)]
...
Perhaps negative adjustments reflect individuals who over-reported their income and thus paid more taxes than they were supposed to.
...
```{r}
# Load Dr. Mills' code for the version we learned in class
source("pairs_ext.r")

# Create scatterplot matrices 
pairs(audit_edu[ , -c(1, 3, 5, 6)], col = as.numeric(audit_edu$TARGET_Adjusted) + 4)
pairs(audit_edu[ , -c(1, 3, 5, 6)], upper.panel = panel.cor, diag.panel = panel.hist)
```
...
ggplot(audit_hourly_wage, 
  ...
  geom_label(data = audit_hourly_wage %>% filter(Hourly_Wage > 1500), 
             ...
             x = 2000, y = 1e5, show.legend = FALSE) + 
...
Self-organizing maps is a method of unsupervised learning. It is described as being akin to k-means clustering, but as a type of dimension reduction, rather than data reduction. 

```{r, eval = FALSE}
# Self-Orgnizing Map    __________________________________________________

# som package used in class notes
install.packages("som")
library(som)

set.seed(007)

# run som function using normalized data
som_aud_s <- som::som(audit_N[,13:17], xdim=5, ydim=5, topol="hexa", init="linear")
plot(som_aud)
```
Honestly, I have no idea how to interpret this plot. It's just the only generic function from her notes that she didn't write, everything else seemed like pedagogical gibberish. 

The kohonen package is also popular for creating self-organizing maps. 
```{r, eval = FALSE}
# kohonen package, popular for SOMs
install.packages("kohonen")
library(kohonen)

g <- kohonen::somgrid(xdim = 5, ydim = 5, topo = "rectangular")
map <- kohonen::som(X = as.matrix(audit_N[,13:17]), grid = g)
plot(map, type = 'changes') # probably won't use this
```
This first plot is sort of a diagnostic showing how many interations were performed. Probably don't care about this but it's interesting to see. 
```{r, eval = FALSE}
plot(map, main = "5x5 code plot of numeric variables in audit data")
```
This code plot shows clusters of different levels of the numeric variables in our dataset, with larger pieces indicating higher levels of a given variable. With respect to the risk adjustment variable, we see one node where moderate amounts of risk adjustment are associated with lower ages and deductions. 
```{r, eval = FALSE}
plot(map, type = 'count') 
```
This plot shows the counts of cases in each node. Maybe this could be displayed next to the code plot. Don't really know what to say about this, I think it really just illustrates how the individual cases are mapped to each node. 
...
ll_null <- glmod1$null.deviance / (-2)
ll_proposed <- glmod1$deviance/ (-2)
 
## McFadden's Pseudo R^2 = [ LL(Null) - LL(Proposed) ] / LL(Null)
(ll_null - ll_proposed) / ll_null
 
## The p-value for the R^2
1 - pchisq(2 * (ll_proposed - ll_null), df = (length(glmod1$coefficients) - 1))
```
...
lm_subsets <- regsubsets(x = RISK_Adjustment ~ ., data = complete_train_audit_m,
